        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card-premium">
                ...
                <div class="p-6 space-y-4 bg-amber-50/30">
                    @php
                        $recentReviews = \App\Models\QualityAssessment::where('tutor_id', $tutor->id)->with(['student', 'assessor'])->latest()->take(5)->get();
                    @endphp
                    @forelse($recentReviews as $review)
                        <div class="bg-white p-4 rounded-xl border {{ $review->is_positive ? 'border-amber-100 shadow-sm' : 'border-red-300 shadow-md shadow-red-100' }}">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <p class="font-bold text-sm text-gray-900">{{ $review->assessor->name ?? 'Orang Tua' }} <span class="text-xs font-normal text-gray-500">(Wali dari {{ $review->student->name }})</span></p>
                                    <div class="flex items-center gap-1 mt-1">
                                        @for($i=1; $i<=5; $i++)
                                        <svg class="w-4 h-4 {{ $i <= $review->overall_score ? 'text-amber-400' : 'text-gray-200' }}" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                        @endfor
                                        <span class="text-xs font-bold ml-1 {{ $review->is_positive ? 'text-amber-600' : 'text-red-500' }}">{{ $review->overall_score }}/5</span>
                                    </div>
                                </div>
                            ...
                            @if($review->requires_followup)
                            <div class="mt-3 text-xs font-bold text-red-700 bg-red-100 border border-red-200 inline-flex items-center gap-1 px-3 py-1.5 rounded-full">
