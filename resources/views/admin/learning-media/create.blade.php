...
<div class="space-y-6 max-w-4xl mx-auto pb-12">
    ...
    <div class="flex items-center gap-3 text-sm text-slate-500 bg-white/50 backdrop-blur-md px-6 py-3 rounded-2xl border border-slate-200/60 shadow-sm w-fit">
        ...
        <span class="text-slate-900 font-bold">Tambah Baru</span>
    </div>
    ...
    <div class="bg-gradient-to-r from-indigo-900 to-slate-900 p-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h1 class="text-3xl font-extrabold tracking-tight mb-2">Tambah Media Baru</h1>
            <p class="text-indigo-200 text-lg">Lengkapi informasi di bawah ini untuk menambahkan materi belajar baru.</p>
        </div>
    </div>

    <!-- Form Section -->
    <form action="{{ route('admin.learning-media.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-[2rem] shadow-xl border border-slate-200/60 overflow-hidden" x-data="{ mediaType: '{{ old('type', 'pdf') }}' }">
        @csrf
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" id="title" class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-colors py-3 px-4 bg-slate-50 hover:bg-white focus:bg-white" value="{{ old('title') }}" required placeholder="Contoh: Modul Matematika Kelas 5">
            ...
            <div class="space-y-6 pt-4">
                ...
                <div x-show="mediaType !== 'pdf'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                    ...
                    <div class="relative">
                        ...
                        <input type="url" name="url" id="url" class="w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-colors py-3 pl-11 pr-4 bg-slate-50 hover:bg-white focus:bg-white" value="{{ old('url') }}" placeholder="https://...">
                    </div>
                    ...
                    <label class="relative flex cursor-pointer rounded-2xl border-2 border-slate-200 bg-white p-5 focus:outline-none hover:border-slate-300 transition-colors">
                        <div class="flex w-full items-center justify-between">
                            ...
                            <div class="ml-4 flex h-6 items-center">
                                ...
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="h-6 w-6 rounded text-indigo-600 focus:ring-indigo-600 transition-colors">
                            </div>
                        </div>
                    </label>
                    ...
                    <label class="relative flex cursor-pointer rounded-2xl border-2 border-slate-200 bg-white p-5 focus:outline-none hover:border-slate-300 transition-colors">
                        <div class="flex w-full items-center justify-between">
                            ...
                            <div class="ml-4 flex h-6 items-center">
                                ...
                                <input type="checkbox" name="is_premium" value="1" {{ old('is_premium') ? 'checked' : '' }} class="h-6 w-6 rounded text-amber-500 focus:ring-amber-500 transition-colors">
                            </div>
                        </div>
                    </label>
                </div>
        ...
        <div class="px-8 py-6 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="px-8 py-3 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-all shadow-md shadow-indigo-200 hover:shadow-lg hover:-translate-y-0.5">
                Simpan Materi Baru
            </button>
        </div>
    </form>
