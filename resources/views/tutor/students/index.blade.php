...
<div class="space-y-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <p class="text-gray-500 text-sm">Kelola data murid bimbingan Anda</p>
        </div>
        <a href="{{ route('tutor.students.create') }}" class="btn-primary-gradient text-white font-bold px-5 py-2.5 rounded-2xl hover:shadow-2xl flex items-center gap-2 shadow-xl shadow-indigo-500/30 w-full sm:w-auto justify-center">
    ...
    <div class="card-premium overflow-hidden">
        ...
        <div class="hidden sm:block overflow-x-auto">
            <table class="table-premium">
                <thead>
                    <tr>
                        <th class="text-left py-4 px-6">Murid</th>
                        <th class="text-left py-4 px-6">Orang Tua</th>
                        <th class="text-left py-4 px-6">Sekolah</th>
                        ...
                        <tr class="group hover:bg-slate-50 transition-colors">
                            ...
                            <td class="py-4 px-6 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('tutor.students.show', $student) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-xl font-semibold text-xs hover:bg-indigo-100 transition-all">
                                    ...
                                    <a href="{{ route('tutor.students.edit', $student) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 text-amber-700 rounded-xl font-semibold text-xs hover:bg-amber-100 transition-all">
                                        <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                        Edit
                                    </a>
                                    <form action="{{ route('tutor.students.destroy', $student) }}" method="POST" class="inline" onsubmit="return confirm('Yakin hapus murid ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="py-12 text-center">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    ...
                                    <p class="text-gray-500 font-semibold">Belum ada murid</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="p-4 space-y-3 hover:bg-slate-50/50 transition-colors">
                    ...
                    <div class="flex items-center justify-between gap-3">
                        <div class="flex items-center gap-3 min-w-0">
                            <img src="{{ $student->photo ? asset('storage/' . $student->photo) : 'https://ui-avatars.com/api/?name=' . urlencode($student->name) . '&background=205085&color=fff&size=128' }}"
                                 class="h-12 w-12 rounded-xl object-cover shadow-md flex-shrink-0">
                            <div class="min-w-0">
                                <p class="font-bold text-slate-800 truncate">{{ $student->name }}</p>
                                @if($student->birth_date)
                                    <p class="text-xs text-slate-500">{{ $student->birth_date->format('d M Y') }}</p>
                                @endif
                            </div>
                        </div>
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold flex-shrink-0
                            @if($student->is_active) bg-green-100 text-green-700
                    ...
                    <div class="bg-slate-50 rounded-xl p-3 space-y-2 border border-slate-100">
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Orang Tua</div>
                                <div class="text-xs font-semibold text-slate-700 truncate">{{ $student->client->user->name }}</div>
                                <div class="text-[10px] text-slate-500 truncate">{{ $student->client->user->phone ?? '-' }}</div>
                            </div>
                            <div>
                        ...
                        <div class="pt-2 border-t border-slate-100">
                            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Sekolah</div>
                            <div class="text-xs text-slate-600">{{ $student->school_name }}</div>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-2 pt-1">
                        <a href="{{ route('tutor.students.show', $student) }}" class="inline-flex items-center justify-center gap-1 py-2 bg-indigo-50 text-indigo-700 rounded-xl font-bold text-xs hover:bg-indigo-100 transition-all">
                            <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                            </svg>
                        ...
                        <a href="{{ route('tutor.students.edit', $student) }}" class="inline-flex items-center justify-center gap-1 py-2 bg-amber-50 text-amber-700 rounded-xl font-bold text-xs hover:bg-amber-100 transition-all">
                            <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                            Edit
                        </a>
                        <form action="{{ route('tutor.students.destroy', $student) }}" method="POST" onsubmit="return confirm('Yakin hapus murid ini?');">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <div class="py-12 text-center">
                    <div class="flex flex-col items-center gap-3">
                        ...
                        <p class="text-gray-500 font-semibold text-sm">Belum ada murid</p>
                    </div>
                </div>
            @endforelse
        </div>
